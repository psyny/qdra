...
export function RecipeCatalogPage({ projectId }: RecipeCatalogPageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Find recipe_catalog view
        const recipeView = templateData.views.find(v => v.view_key === 'recipe_catalog');
        if (!recipeView) {
          setError('Recipe catalog view not found in template');
          return;
        }
        
        setRecipeCatalogView(recipeView);
        
        // Auto-select if only one config or if configId is in URL
        if (urlConfigId) {
          const config = recipeView.configs.find(c => c.id === urlConfigId);
          if (config) {
            setSelectedConfig(config);
          }
        } else if (recipeView.configs.length === 1) {
          setSelectedConfig(recipeView.configs[0]);
        }
      } catch (err) {
  ...
  if (loading || entitiesLoading) {
    return <LoadingSpinner message={loading ? "Loading recipe catalog..." : "Loading recipes..."} />;
  }
              ...
              <div className="material-catalog-card__image-region">
                {entity.image ? (
                  <img 
                    ...
                    alt={entity.image.alt_text || getDisplaySlotValue(entity, 0) || 'Recipe'}
      ...
      {deleteConfirmEntity && (
        <div className="modal-overlay">
          <div className="card" style={{ maxWidth: '400px', margin: 'auto' }}>
            <h3 className="card-title mb-4">Delete Recipe</h3>
            <p className="mb-4">
