...
export async function getMaterials(projectId: string): Promise<Material[]> {
  const response = await fetch(apiUrl(`/api/projects/${projectId}/materials`));
  if (!response.ok) {
...
export async function getMaterial(projectId: string, materialId: string): Promise<Material> {
  const response = await fetch(apiUrl(`/api/projects/${projectId}/materials/${materialId}`));
  if (!response.ok) {
...
export async function addMaterialParameter(
  ...
  const response = await fetch(apiUrl(`/api/projects/${projectId}/materials/${materialId}/parameters`), {
    method: 'POST',
