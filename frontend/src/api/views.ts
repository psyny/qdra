...
export async function getTemplateViews(templateId: string): Promise<View[]> {
  const response = await fetch(apiUrl(`/api/project-templates/${templateId}/views`));
...
export async function updateView(templateId: string, viewId: string, payload: UpdateViewRequest): Promise<View> {
  const response = await fetch(apiUrl(`/api/project-templates/${templateId}/views/${viewId}`), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
export async function seedSystemViews(templateId: string): Promise<View[]> {
  const response = await fetch(apiUrl(`/api/project-templates/${templateId}/views/seed-system`), {
    method: 'POST',
  });
...
export async function createViewConfig(
  ...
  const response = await fetch(apiUrl(`/api/project-templates/${templateId}/views/${viewId}/configs`), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
...
export async function updateViewConfig(
  ...
  const response = await fetch(
    apiUrl(`/api/project-templates/${templateId}/views/${viewId}/configs/${configId}`),
    {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
